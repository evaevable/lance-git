...
'''
    在主解析器上添加了一个子解析器（subparser）。子解析器允许你定义多个子命令，每个子命令可以有自己的参数和选项。
    title参数用于设置子解析器的标题。这个标题通常用于生成帮助信息，以描述这一组子命令的总体功能或目的。
    dest参数用于指定一个字符串，该字符串将作为属性名，用于在解析命令行参数后存储所选子命令的名称。
'''
argsubparsers = argparser.add_subparsers(title="Commands", dest="command")
argsubparsers.required = True

'''
    sys.argv 是 Python 的一个内置模块 sys 中的一个属性，它是一个列表，包含了命令行参数，
    sys.argv[0] 是脚本的名称（它可能是一个脚本路径，取决于如何启动脚本）。
    sys.argv[1:] 是传递给脚本的所有参数。
    例如，如果你在命令行中运行以下命令：
...
class GitRepository(object):
    ...
    def __init__(self, path, force=False):
        ...
        # 读取配置.git/config
        # ConfigParser 可以读取 INI 格式的配置文件。INI 文件是一种简单的文本文件，用于存储键值对和分组信息。
        self.conf = configparser.ConfigParser()
        cf = repo_file(self, "config")

        if cf and os.path.exists(cf):
            # read 方法会解析指定的配置文件，并将解析得到的配置数据存储在 ConfigParser 对象中。
            self.conf.read([cf])
        elif not force:
...
def repo_find(path=".", required=True):
    """
        返回指定路径的规范化的绝对路径
        解析符号链接：如果路径中包含符号链接（也称为软链接或快捷方式），realpath 会解析它们，返回链接指向的实际文件或目录的路径。
        规范化路径：realpath 会将路径中的 . 和 .. 等相对路径元素解析为绝对路径，并去除多余的斜杠和尾随斜杠。
        返回绝对路径：最终，realpath 返回的是规范化的绝对路径。
    """
...
class GitObject(object):
    ...
    def serialize(self, repo):
        """
        此函数必须由子类实现。它必须从 self.data（一个字节字符串）中读取对象的内容，
        并尽一切努力将其转换为有意义的表示形式。这究竟意味着什么取决于每个子类。
...
def object_read(repo, sha):
    """
    从 Git 存储库存储库中读取对象 sha。返回一个 GitObject，其确切类型取决于该对象。
...
def kvlm_parse(raw, start=0, dct=None):
    if not dct:
        dct = dict()
        # 你不能将参数声明为,dct = dict()，否则对函数的所有调用都会无休止地增长同一个字典。

    # 这个函数是递归的,它读取一个键值对,然后用新位置调用自己。
    # 所以我们首先需要知道我们在哪里: at a keyword, or already in the messageQ

    # 我们搜索下一个空格和下一个换行符。
    spc = raw.find(b' ', start)
    nl = raw.find(b'\n', start)

    # 如果换行符之前出现空格，则我们有一个关键字。
    # 否则，它是最后一条消息，我们只需将其读取到文件末尾。
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'lance <lance@example.com>'
        # …and a tag message!
        tag.kvlm[None] = b"A tag generated by lance, which won't let you customize the message!\n"
...
def index_read(repo):
    ...
    assert version == 2, "lance only supports index file version 2"
...
def add(repo, paths, delete=True, skip_missing=False):
    ...
    for (abspath, relpath) in clean_paths:
        with open(abspath, "rb") as fd:
            ...
            entry = GitIndexEntry(ctime=(ctime_s, ctime_ns), mtime=(mtime_s, mtime_ns), dev=stat.st_dev,
                                  ino=stat.st_ino,
